...
namespace MotionActivatedLight {
  ...
  Svc::InputSchedPort *MotionSensorComponentBase ::
  ...
  void MotionSensorComponentBase ::
    set_timeOut_OutputPort(
        NATIVE_INT_TYPE portNum,
        MotionActivatedLight::InputPortTimePort* port
    )
  {
    FW_ASSERT(portNum < this->getNum_timeOut_OutputPorts(),static_cast<FwAssertArgType>(portNum));
    this->m_timeOut_OutputPort[portNum].addCallPort(port);
  }
  ...
  {
    FW_ASSERT(portNum < this->getNum_timeOut_OutputPorts(),static_cast<FwAssertArgType>(portNum));
    return this->m_timeOut_OutputPort[portNum].registerSerialPort(port);
  }
    ...
    init(NATIVE_INT_TYPE instance)
  {
    ...
    // Initialize output port timeOut
    for (
        PlatformIntType port = 0;
        port < static_cast<PlatformIntType>(this->getNum_timeOut_OutputPorts());
        port++
    ) {
      this->m_timeOut_OutputPort[port].init();
      ...
      (void) snprintf(
          portName,
          sizeof(portName),
          "%s_timeOut_OutputPort[%" PRI_PlatformIntType "]",
          this->m_objName,
          port
      );
      this->m_timeOut_OutputPort[port].setObjName(portName);
#endif
  ...
  {
    FW_ASSERT(portNum < this->getNum_timeOut_OutputPorts(),static_cast<FwAssertArgType>(portNum));
    this->m_timeOut_OutputPort[portNum].invoke(currentTime, lastDetectedTime);
  }
  ...
  NATIVE_INT_TYPE MotionSensorComponentBase ::
    getNum_timeOut_OutputPorts()
  {
    return static_cast<NATIVE_INT_TYPE>(FW_NUM_ARRAY_ELEMENTS(this->m_timeOut_OutputPort));
  }
  ...
  bool MotionSensorComponentBase ::
    isConnected_timeOut_OutputPort(NATIVE_INT_TYPE portNum)
  {
     FW_ASSERT(
        portNum < this->getNum_timeOut_OutputPorts(),
        static_cast<FwAssertArgType>(portNum)
     );
     return this->m_timeOut_OutputPort[portNum].isConnected();
  }

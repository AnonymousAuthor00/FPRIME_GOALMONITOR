...
#include <Os/Log.hpp>
#include <example/Top/exampleTopologyAc.hpp>
...
static void sighandler(int signum) {
    example::teardown(state);
    terminate = 1;
}

void run1cycle() {
    // call interrupt to emulate a clock
    example::blockDrv.callIsr();
...
int main(int argc, char* argv[]) {
    ...
    state = example::TopologyState(hostname, port_number);
    example::setup(state);
